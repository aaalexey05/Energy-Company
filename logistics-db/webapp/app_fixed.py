...
def orders():
    ...
    for order in orders_list:
        ...
        status_color = status_colors.get(order['статус_груза'], '#95a5a6')
        ...
        <tr>
            ...
            <td><span style="padding:5px 10px;background:{status_color};color:white;border-radius:5px;font-size:12px">{order['статус_груза']}</span></td>
...
def routes():
    ...
    for route in routes_list:
        ...
        <tr>
            ...
            <td><strong>{route['название']}</strong></td>
...
def vehicles():
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute('SELECT * FROM транспортные_средства ORDER BY ид_транспорт')
    ...
    for vehicle in vehicles_list:
        actions = ""
        if can_edit:
            actions += f'<a href="/admin/vehicles/edit/{vehicle["ид_транспорт"]}" style="margin-right:10px;color:#3498db">✏️ Изменить</a>'
        if can_delete:
            actions += f'<a href="#" onclick="if(confirm(\'Удалить?\')){{fetch(\'/admin/vehicles/delete/{vehicle["ид_транспорт"]}\',{{method:\'POST\'}}).then(()=>location.reload())}}; return false;" style="color:#e74c3c">🗑️ Удалить</a>'
        ...
        content += f"""
        <tr>
            <td>{vehicle['ид_транспорт']}</td>
            <td><strong>{vehicle['гос_номер']}</strong></td>
            <td>{vehicle['марка']}</td>
